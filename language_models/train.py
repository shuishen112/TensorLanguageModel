...
from dataloader import PADDING_TOKEN, DatasetFactory
from model import CharRNN
from torch.utils import data
from util import print_tokens

from language_models.lightRNN import JitRNN_language_model, RNN_language_model
...
def get_model(dataset, config):
    # return CharRNN(
    #     vocab_size=dataset.encoder.vocabulary_size,
    ...
    return JitRNN_language_model(
        vocab_size=dataset.encoder.vocabulary_size,
        embed_size=config["model_config"]["embedding_size"],
        hidden_size=config["model_config"]["hidden_size"],
        padding_idx=dataset.encoder.token_to_id(PADDING_TOKEN),
    )
...
def run_training(model, dataset, config: dict, validation: bool, valid_dataset):
    ...
    while epoch < config["epochs"] + 1:
        ...
        for step, (x_i, y_i, l_i) in enumerate(data_generator):
            ...
            if step % 100 == 0:
                ...
                print_tokens(
                    dataset.encoder.map_ids_to_tokens(gen_sample), config["is_bytes"]
                )
        ...
        if epoch in config["lr_schedule"]:
            optimizer = torch.optim.Adam(
                model.parameters(), lr=config["lr_schedule"][epoch]
            )
        epoch += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train a LSTM network defined in model.py."
    )
    parser.add_argument(
        "-d",
        "--dataset",
