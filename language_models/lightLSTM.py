...
from language_models.util import coalesce
from utils.clm_data_process import Encoder


class litCharRNN(LanguageBaselightning):
    def __init__(
        self,
        vocab_size: int,
        embedding_size: int,
    ...
    def forward(self, x, lengths):
        ...
        if self.hidden[0].size(1) > batch_size:
            warnings.warn(
                f"Expected batch_size {self.hidden[0].size(1)}, but received {batch_size} "
                f"instead. Trimming the hidden state to match the batch size"
            )
            self.hidden = (
                self.hidden[0][:, :batch_size, :].contiguous(),
    ...
    def sample(probabilities):
        return int(
            np.random.choice(
                len(probabilities), size=1, p=probabilities.data.cpu().numpy()
            )[0]
        )

    def generate_text(
        self, encoder: Encoder, starting_seq: Union[list, str], sample_size: int
    ):
        """
    ...
    def init_hidden(self, batch_size):
        return (
            Variable(torch.zeros(self.n_layers, batch_size, self.hidden_size)).to(
                self.device
            ),
            Variable(torch.zeros(self.n_layers, batch_size, self.hidden_size)).to(
                self.device
