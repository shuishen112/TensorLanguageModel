def _get_filename(dataset_name, config):
    ...
    return dataset_name + "_{e}_epochs_{h}_{nl}_layers_{t}_timesteps".format(
        e=config["epochs"],
        h=config["model_config"]["hidden_size"],
        nl=config["model_config"]["n_layers"],
...
text8config = {
    ...
    "epochs": 51,
    "initial_lr": 0.001,
    "lr_schedule": {25: 0.0005, 39: 0.0002},
    "model_config": {"embedding_size": 64, "hidden_size": 512, "n_layers": 2},
    "is_bytes": False,
}
text8config["filename"] = _get_filename("text8", text8config)
text8config["tn_filename"] = _get_filename("tn_text8", text8config)
...
ptbconfig = {
    ...
    "epochs": 60,
    "initial_lr": 0.001,
    "lr_schedule": {40: 0.0001, 50: 0.00002},
    "model_config": {"embedding_size": 16, "hidden_size": 512, "n_layers": 2},
    "is_bytes": False,
}
